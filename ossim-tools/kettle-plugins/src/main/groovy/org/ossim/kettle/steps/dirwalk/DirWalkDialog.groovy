...
class DirWalkDialog extends BaseStepDialog implements
        StepDialogInterface
{
   private DirWalkMeta input
   private KettleSwtBuilder swt;
   private Integer currentRowNumberToEdit = -1
   public DirWalkDialog(Shell parent, Object baseStepMeta,
                         TransMeta transMeta, String stepname)
   {
      super(parent, (BaseStepMeta)baseStepMeta, transMeta, stepname);
      input = (DirWalkMeta)baseStepMeta;
   }
   public String open()
   {
      ...
      colinf[4] = new ColumnInfo("File Type", ColumnInfo.COLUMN_TYPE_CCOMBO, ["ALL", "DIRECTORY", "FILE"] as String[], true);
      ...
      shell = swt.shell(parent) {
         ...
         group(id: "stepNameGroup") {
            ...
            label Messages.getString("DirWalkDialog.Stepname.Label")
         ...
         group(layoutData: "grow, span, wrap") {
            ...
            button(id: "okButton", Messages.getString("DirWalkDialog.ok.Label"),
                    layoutData: "align center,split 2") {
               onEvent(type: "Selection") { ok() }
            }
            button(id: "cancelButton", Messages.getString("DirWalkDialog.cancel.Label"), layoutData: "") {
               onEvent(type: "Selection") { cancel() }
            }
         }
      }
      changed = input.hasChanged();
      shell.text = Messages.getString("DirWalkDialog.Shell.Title")
   ...
   private fileInputFromFieldCheckboxModified()
   {
      ...
      swt.fieldFileType.enabled          = swt.fileInputFromField.selection
      ...
      swt.fileType.enabled          = !swt.fileInputFromField.selection
      ...
      if(itemCount>0)
      {
         ...
         if(currentRowNumberToEdit>=0)
         {
            ...
            if(item)
            {
               ...
               swt.fileType.text               = item.getText(5)
            }
         }
      }
   }
   private void addRow(){
      if(swt.filename?.text)
      {
         ...
         item?.setText(5, swt.fileType?.text?:"FILE")
   ...
   private void loadFileinfoTable() {
      ...
      if(fields?.size())
      {
         ...
         fields?.eachWithIndex{fieldDefinition, i->
            ...
            item?.setText(5, fieldDefinition.fileType?:"FILE")
         }
      }
   }
   ...
   private void saveFileinfoTable()
   {
      ...
      (0..<itemCount).each { idx ->
         ...
         String fileType          = "${item.getText(5)}".trim()
   ...
   private getData()
   {
      ...
      swt.fieldFileType.text            = input.fieldFileType?:""
   ...
   private void ok()
   {
      ...
      input.fieldFileType            = swt.fieldFileType.text
      saveFileinfoTable()
