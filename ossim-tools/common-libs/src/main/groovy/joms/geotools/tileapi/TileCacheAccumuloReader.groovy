 ...
 * @source $URL$
 */
public class TileCacheAccumuloReader extends AbstractGridCoverage2DReader {
  private final static Logger LOGGER = Logging.getLogger(TileCacheAccumuloReader.class.getPackage()
          .getName());
 ...
 // private final JDBCAccess jdbcAccess;
  ...
  private TileCacheConfig config;
  private TileCacheHibernate hibernate
  private TileCacheServiceDAO tileCacheServiceDAO
   ...
   */
  public TileCacheAccumuloReader(Object source, Hints uHints) throws IOException,
          MalformedURLException {
    this.source = source;

    config = null

    config = new TileCacheConfig()
    try{
    ...
    {
      throw new MalformedURLException("Unable to parse input");
    }

    hibernate = new TileCacheHibernate()
    hibernate.initialize(config.connectionParams())
    tileCacheServiceDAO = hibernate.applicationContext.getBean("tileCacheServiceDAO");
 ...
 /*
    ...
    if (tempCRS != null) {
      ...
      LOGGER.log(Level.WARNING, new StringBuffer("Using forced coordinate reference system ")
  ...
  */
  ...
  void initialize()
  {
    TileCacheLayerInfo layerInfo = tileCacheServiceDAO.getLayerInfoByName(config.layer)

    if(layerInfo)
    {
      Envelope env = layerInfo.bounds.envelopeInternal

      this.originalEnvelope = new GeneralEnvelope(new Rectangle2D.Double(env.minX, env.minY, env.width, env.height));
      this.crs = CRS.decode(layerInfo.epsgCode, false);
    }
   ...
   *
   */
  public TileCacheAccumuloReader(Object source) throws IOException {
    this(source, null);
  }
  ...
  public GridCoverage2D read(GeneralParameterValue[] params) throws IOException {
    ...
    println "READING A GRID COVERAGE!!!!!!!!!!!!!!!!!!!!!!${params}"
    /*
   // ImageMosaicJDBCReaderState state = new ImageMosaicJDBCReaderState();
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      ...
      LOGGER.fine(new StringBuffer("Highest res ").append(highestRes[0]).append(" ").append(
   ...
   //         + " millisecs");
     println "NEED TO ADD GRID COVERAGE!!!!!!!!!!!!!!!"
    return coverage;
    */
    println "NEED TO ADD GRID COVERAGE!!!!!!!!!!!!!!!"
 ...
 /* private void transformRequestEnvelope(ImageMosaicJDBCReaderState state)
    ...
    if (CRS.equalsIgnoreMetadata(state.getRequestedEnvelope().getCoordinateReferenceSystem(),
            this.crs)) {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.fine(new StringBuffer("Creating mosaic to comply with envelope ").append(
              (state.getRequestedEnvelope() != null) ? state.getRequestedEnvelope()
                      .toString() : null).append(" crs ").append(crs.toWKT()).append(" dim ")
...
//                    .getRequestedEnvelope());
  ...
  /*
  private GridCoverage2D transformResult(GridCoverage2D coverage, Rectangle pixelDimension,
    ...
    // coverage.show();
    result = (GridCoverage2D) Operations.DEFAULT.resample(coverage, state
            .getRequestedEnvelope().getCoordinateReferenceSystem());
    // result.show();
    ...
    return coverageFactory.create(result.getName(), result.getRenderedImage(), result
            .getEnvelope());
