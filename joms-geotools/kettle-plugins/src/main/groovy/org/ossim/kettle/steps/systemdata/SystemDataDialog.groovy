...
{
   private static final String PKG = SystemDataMeta.package.name; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$
	...
	public SystemDataDialog(Shell parent, Object baseStepMeta,
										TransMeta transMeta, String stepname) {
		super(parent, (BaseStepMeta)baseStepMeta, transMeta, stepname);
		input=(SystemDataMeta)baseStepMeta;
	}
	...
	public String open()
	{
      ...
      final String[] functionDesc = new String[SystemDataMeta.functions.size()-1];
      for (int i=1;i<SystemDataMeta.functions.length;i++) functionDesc[i-1]=SystemDataMeta.functions[i].description;
		    ...
		    new SelectionAdapter()
	        {
	            public void widgetSelected(SelectionEvent e)
	            {
	                EnterSelectionDialog esd = new EnterSelectionDialog(shell, 
	                	functionDesc, 
	                	BaseMessages.getString(PKG, "SystemDataDialog.SelectInfoType.DialogTitle"), BaseMessages.getString(PKG, "SystemDataDialog.SelectInfoType.DialogMessage"));
		...
		setButtonPositions([wOK, wPreview, wCancel] as Button[], margin, wFields);
	...
	private void getInfo(SystemDataMeta systemDataMeta) throws KettleException
	{

		stepname = wStepname.getText(); // return value
		int count = wFields.nrNonEmpty();
		systemDataMeta.allocate(count);
		
		for (int i=0;i<count;i++)
		{
			TableItem item = wFields.getNonEmpty(i);
			systemDataMeta.getFieldName()[i]   = item.getText(1);
			systemDataMeta.getFieldType()[i]   = SystemDataMeta.getType(item.getText(2));
		}
	}
	// Preview the data
	private void preview()
	{
        try
        {
            ...
            if (previewSize>0)
            {
                TransPreviewProgressDialog progressDialog = new TransPreviewProgressDialog(shell, previewMeta, [wStepname.getText()] as String[], [previewSize] as int[] );
                progressDialog.open();
